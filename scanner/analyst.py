...
def _note_payload(c: dict, headlines: list[dict], regime_light: str | None = None,
    ...
    return {
        "task": (
            ...
            "episodic_pivot (if present) BACKTESTS NEGATIVE on Bursa: -0.38R "
            "expectancy over 292 trades with a -75% drawdown (measured on the full "
            "universe). Treat it as a HAZARD, not a setup. Its catalyst must be "
            "verified by search — earnings, a contract award, a corporate action. "
            "With a verified catalyst it may be discussed as speculative and risk is "
            "still 'high'. With NO findable catalyst, verdict='avoid' and say the gap "
            "is unexplained: on this market an unexplained gap on huge volume is an "
            "operator signature, and the measured edge is negative. Never write a "
            "buy plan around an EP alone. "
